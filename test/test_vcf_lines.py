...
def test_gnomad_sites_small(tmp_path):
    ...
    query = (
        "SELECT rowid, line FROM"
        + genomicsqlite.overlapping_genomic_ranges(con, "gnomad_vcf_lines")
    )
    ...
    control = (
        "SELECT rowid FROM gnomad_vcf_lines WHERE NOT ((POS+rlen) < ? OR POS > ?)"
    )
...
def test_pvcf_gatk(tmp_path):
    ...
    vcf_lines_into_sqlite(
        os.path.join(HERE, "data/gatk.1KGP.ALDH2.vcf.gz"), str(dbfile)
    )
    rows = 2088
    con = genomicsqlite.connect(dbfile, read_only=True)
    assert next(con.execute("SELECT COUNT(*) FROM vcf_lines"))[0] == rows
